-- Opravená verze mydb.sql s lepší strukturou
-- Datum: 2025-04-26
-- Upraveno pro kompatibilitu s frontendem

-- Vytvoření databáze (pokud neexistuje)
CREATE DATABASE IF NOT EXISTS `dad_db` CHARACTER SET utf8mb4 COLLATE utf8mb4_czech_ci;
USE `dad_db`;
...
CREATE TABLE `unread_notifications` (
  ...
  PRIMARY KEY (`id`),
  KEY `unread_notifications_fk_ticket` (`ticket_id`),
  KEY `unread_notifications_fk_user` (`user_id`),
  CONSTRAINT `unread_notifications_fk_ticket` FOREIGN KEY (`ticket_id`) REFERENCES `tickets` (`id`) ON DELETE CASCADE,
  CONSTRAINT `unread_notifications_fk_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- --------------------------------------------------------
-- Data z mydb.sql (produkční data)
-- POZOR: Někteří uživatelé mají prázdné heslo - je potřeba je opravit!

-- Uživatelé (bez těch s prázdným heslem - ty je potřeba vytvořit znovu)
...
-- POZOR: Uživatelé 13, 15, 16, 17, 18 mají prázdné heslo - je potřeba je vytvořit znovu přes formulář!
...
-- Ticket replies (všechny z mydb.sql)
-- ... (všechny 68 replies z mydb.sql)
